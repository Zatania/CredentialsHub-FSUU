...
const DeleteSA  = ({sa, refreshData}) => {
  const [show, setShow] = useState<boolean>(false)

  const handleDeleteClick = () => {
    axios.delete(`/api/admin/student_assistant/delete/${sa.id}`)
      .then(() => {
        toast.success('Student Assistant deleted successfully');
        refreshData(); // Assuming this function refreshes the list of Student Assisntants
      })
      .catch((error) => {
        console.error(error);
