...
const DialogViewTransaction = ({ transaction, refreshData }: DialogViewTransactionProps) => {
  ...
  const handleSchedule = async () => {
    ...
    try {
      await axios.put(`/api/student_assistant/transactions/${transaction.id}/schedule`, {
  ...
  const handleReject = async () => {
    if (!validateRemarks()) return
    try {
      await axios.put(`/api/student_assistant/transactions/${transaction.id}/reject`, {
  ...
  const handleClaim = async () => {
    if (!validateRemarks()) return
    try {
      await axios.put(`/api/student_assistant/transactions/${transaction.id}/claim`, {
  ...
  const handleDone = async () => {
    try {
      await axios.put(`/api/student_assistant/transactions/${transaction.id}/done`, {
        user: user
      })
  ...
  const handleUpload = async () => {
    ...
    try {
      if (path && selectedDate) {
        await axios.put(`/api/student_assistant/transactions/${transaction.id}/update`, {
          imagePath: path,
