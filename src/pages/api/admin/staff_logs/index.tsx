...
async function fetchStaffLogs() {
  const results = (await db.query(`
    SELECT
      ...
      END AS action_date,
      COALESCE(GROUP_CONCAT(DISTINCT CASE WHEN td.quantity > 0 THEN c.name END ORDER BY c.name), GROUP_CONCAT(DISTINCT cp.name ORDER BY c.name)) AS credentials_requested,
      CASE
