...
async function fetchSALogs() {
  const results = (await db.query(`
    SELECT
      t.id AS id,
      t.transaction_date AS timestamp,
      CONCAT(s.firstName, ' ', s.lastName) AS sa,
    ...
    JOIN student_assistants s ON th.sa_id = s.id
  ...
  const rows = results[0].map((row: SALogsData) => ({
    id: row.id,
...
export default async function handler(
  ...
  if (req.method === 'GET') {
    try {
      const credentials = await fetchSALogs()
