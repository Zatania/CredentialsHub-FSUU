...
async function fetchSADepartments(sa_id: number) {
  const [results] = (await db.query(`
    SELECT department_id FROM student_assistants_departments WHERE sa_id = ?
  `, [sa_id])) as RowDataPacket[]
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const sa_id = session.user.id

  if (req.method === 'GET') {
    try {
      const allTransactions = {}
      const departments = await fetchSADepartments(sa_id)
