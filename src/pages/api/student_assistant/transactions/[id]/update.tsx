...
async function updateTransaction(id: number, data: any, user: any) {
  try {
    if(data.payment_date && data.imagePath) {
      ...
      const message = `${user.firstName} ${user.lastName} has updated a transaction.`
      await db.query(`INSERT INTO student_assistants_logs (sa_id, activity, activity_type, date) VALUES (?, ?, ?, ?)`, [user.id, message, 'Update Transaction', dayjs().format('YYYY-MM-DD HH:mm:ss')])
